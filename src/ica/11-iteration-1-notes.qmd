...
```{r}
df_arg_combos <- crossing(
  mod_formula = c(price ~ carat + cut, price ~ carat+cut+clarity, price ~ carat+ cut+ clarity + color),
...
```{r}
remove_outliers <- function(data, mod_formula, remove_outliers){
  if(remove_outliers){
...
remove_outliers <- function(data, what_var, sd_thresh){
  data %>%
    mutate(zscroe = ({{what_var}} - mean({{what_var}}, na.rm = TRUE))/sd({{what_var}}, na.rm = TRUE)) %>%
