...
phone_number <- function(number){
  nums <- str_remove_all(number, "[^[:alnum]]")
  area <- str_sub(number, 1, 3)
  num3 <- str_sub(number, 4, 6)
  num4 <- str_sub(number, 7, 10)
  str_c("(",area,") ", num3, "-", num4)
}

phone_number(9169434140)
```
...
The error is that aesthetics goes before data in the function geom_sf, thus the function is mistaking census data as an aesthetic. 
...
extract_domain("gnieves@mac.edu")
extract_domain("gnieves@@@")
```
