...
library(USAboundariesData) # access to boundaries for US states, counties, zip codes, and congressional districts 
library(plotly)
library(babynames)
```
...
I would expect the point data to not be accurate since it would not have the same centroid and ellipsoid as the NAD27 CRS. 
...
mn_counties <- st_transform(mn_counties, crs = 26915)
...
ggsave(filename = "tc_zoom.png", plot = twin_cities_plot, width = 6, height = 4)
```
...
ggsave(filename = "../../figures/tc_zoom.png", plot = twin_cities_plot, width = 6, height = 4)
