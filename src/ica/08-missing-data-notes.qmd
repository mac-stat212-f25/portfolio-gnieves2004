...
If the people who dropped out of the study are considered to be random, then MNAR would be the best mechanism. 

  b. A weather station records temperature, humidity, and wind speed every hour. Some of the recorded values are missing.
  
If the missing values are from technical glithces then it is best to use MCAR.
...
The best mechanism will depend on whether the bugs affected groups of users differently but where the groupings are known or if the bugs lead to performance issues that affect users with slower internet more. The former would be better suited for MAR and the latter for MNAR. 
...
In both versions ozone is missing when solar is at higher values. 
