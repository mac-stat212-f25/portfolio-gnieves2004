...
server <- function(input, output) {
    output$plot_over_years <- renderPlot({ # Note the curly braces that enclose the R code below
        filter(metro_name == "selected_city") %>%
        ggplot(aes(x = entropy, y = distmiles), data = data_by_dist) +
        geom_point() +
        geom_smooth(method ="loess", span = "selected_span") +
        labs(x = "Entropy", y = "Distance in Miles", title = "Entropy vs. Distance")
    })
}
```
